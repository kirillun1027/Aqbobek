...
import { getEvents, getRankings, getAllAchievements } from "@/lib/services/data-service"
...
export default function KioskPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      const [eventsData, rankingsData, achievementsData] = await Promise.all([
        getEvents(true),
        getRankings(),
        getAllAchievements(),
      ])
      ...
      const achievementsWithNames = achievementsData.slice(0, 4).map(a => ({
        ...a,
        studentName: rankingsData.find(r => r.studentId === a.student_id)?.name || "Student",
      }))
      setRecentAchievements(achievementsWithNames as (Achievement & { studentName?: string })[])
    }
