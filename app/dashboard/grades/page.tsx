...
import { getGradesForStudent, calculateSubjectAverage } from "@/lib/services/data-service"
...
export default function GradesPage() {
  ...
  useEffect(() => {
    async function loadGrades() {
      ...
      // For parents, use linked student
      const studentId = user.role === "parent" 
        ? (user.linked_student_ids?.[0] || "student-1")
        : user.id
      
      const [q3Grades, allStudentGrades] = await Promise.all([
        getGradesForStudent(studentId, 3),
