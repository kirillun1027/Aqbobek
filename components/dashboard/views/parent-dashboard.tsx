...
export function ParentDashboard({ user }: ParentDashboardProps) {
  ...
  useEffect(() => {
    async function loadData() {
      // Get linked child (for demo, use first linked student)
      const childId = user.linked_student_ids?.[0] || "student-1"
      const childData = await getStudentById(childId)
      
      if (childData) {
        setChild(childData)
        
        const [gradesData, achievementsData, eventsData, rankingsData] = await Promise.all([
          getGradesForStudent(childData.id, 3),
          getAchievementsForStudent(childData.id),
          getEvents(true),
          getRankings(),
        ])
        ...
        const childRank = rankingsData.findIndex(r => r.studentId === childData.id) + 1
